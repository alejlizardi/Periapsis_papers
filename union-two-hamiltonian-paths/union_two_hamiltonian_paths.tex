...
\hypersetup{
  pdftitle={Properly colored paths in the union of two Hamiltonian paths: from a refuted half-bound to an O(sqrt n) collapse},
  pdfauthor={Alejandro Lizardi},
  pdfsubject={Combinatorics; properly colored paths in edge-colored multigraphs},
  pdfkeywords={properly colored path; alternating path; edge-colored multigraph; union of Hamiltonian paths; permutation; inflation; transit number; Lovasz problem; vertex-transitive graphs}
}
...
$n=12$, suggested the half-bound $\rho(\sigma)\ge\lceil(n+3)/2\rceil$; we exhibit an
...
ancillary code and logs. The asymptotic bound and its constants are proved by elementary,
self-contained arguments that use no computation; the computations independently confirm
them and supply the exact small-case values (the $n=18$ counterexample below is
computer-assisted: its longest properly colored path is found by exhaustive search).
\end{abstract}
...
The asymptotic main theorem and its constants are proved without computation; the one
proof that is computer-assisted is the exact value $\rho(\sigma^\ast)=10$ in the $n=18$
counterexample (Theorem~\ref{thm:n18}), whose upper bound is obtained by exhaustive
search. This section specifies the computations that independently verify the
constructions and supply the exact small-case values, and records the empirical
sharpness of the family.
...
is a classification (Theorem~\ref{thm:classification}) whose every step is locally
verifiable; every computational claim is reproducible from the ancillary artifact, which
includes an independent verifier written directly from the definitions; and the main
theorem has a second proof in Appendix~\ref{app:trap}. This is an unrefereed preprint;
no journal referee has yet checked it.
