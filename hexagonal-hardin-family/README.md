...
Working paper (DRAFT, unrefereed, not yet reviewed by a specialist in the area). Proves the empirical
...
- `hexagonal_hardin_family.pdf` — compiled draft (8 pp).
...
## Honest status (read before circulating)
- **Computer-assisted; unrefereed; not yet reviewed by a specialist in the area.** The author (a
  mathematician, not a specialist in P-partitions / order-polynomial enumeration) has read it, and a
  fresh-agent adversarial audit cleared it; neither substitutes for peer review by a domain expert.
...
- `hexagonal_hardin_family.tex` / `.pdf` — © the author(s); all rights reserved pending circulation.
