...
| open-problems §(matching side) | exact f(n): f = n for n ≤ 6, f(7) = 6, f(8) = 7, f(9) = 8; max_π μ ∈ {n−1, n} for all σ, n ≤ 9, with {max μ = n−1} = D(n); exact max_π μ on σ_{a,6} (a = 2..5) and σ_{3,8} (one edge lost per gadget copy beyond the second) | `fmu.cpp` (bitmask DP; validated vs direct search on all S_n, n ≤ 7, and vs the conversion inequality pointwise); `fmu_cpsat.py` (CP-SAT, optimality certified; needs `ortools`) | (rerun to reproduce) |
