...
Three further problems, each with exact data behind it, close the paper. A third
ordering repairs every small example: any union of three Hamiltonian paths on at
most ten vertices has a properly colored Hamiltonian path, and the smallest
counterexample, unique up to symmetry, is a triple of orderings of eleven points
whose union misses a properly colored Hamiltonian path by exactly one vertex.
Whether $\pminK{3}(n) \to \infty$, and whether the collapse persists for every
fixed number of paths, is open. The collapse also does not transfer to the
matching side: a properly colored path in $H_\sigma$ converts into a path in a
ladder graph with $\sigma$-permuted rungs, using one rung per vertex, and our
family, despite $\rho = \Theta(\sqrt n\,)$, retains high rung content at every
point we can compute exactly. We conjecture that the minimum over $\sigma$ of the
best rung count, $f(n)$, grows linearly in $n$; how close it stays to $n$ is
open. Finally, the exact value $\rho(\sigma_{a,b}) = 2a + 2b - 4$, computed at
every tested point up to $n = 720$, is conjectured for the whole gadget family.
\end{abstract}
