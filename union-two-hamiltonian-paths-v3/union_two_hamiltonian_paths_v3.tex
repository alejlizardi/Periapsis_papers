...
\hypersetup{
  ...
  pdfkeywords={properly colored path; alternating path; edge-colored multigraph; union of Hamiltonian paths; permutation; inflation; transit number; block-free permutation; scrambled ladder}
}
...
reported cases); and an edge-by-edge verifier for the chain snake
(\texttt{trap\_snake.py}).
...
$\pmin(n) = \Theta(\sqrt n)$.
%% [E8 call-6 2026-07-28] conj:sqrt demoted from displayed conjecture to prose;
%% conj:exactgadget (below) is the paper's displayed conjecture. Stale
%% \ref{conj:sqrt} site later in this section: recorded in ledger E10.
...
has exactly $n-1$ non-matching edges, one between consecutive matching edges;
consecutive connector edges must alternate between the top and bottom paths,
...
On the negative side, the collapse of $\rho$ does \emph{not} transfer: both
families of this paper have nearly full matching content at every point we can
compute exactly. For the gadget family, exact values (integer programming,
...
$\ge n - 3a$ of \texttt{verify\_f\_linear.py} is far from optimal. The chains
of Appendix~\ref{app:trap} behave identically: an explicit snake
(machine-verified edge by edge at sizes up to $n = 5201$) uses at least
$n - \#\{\text{odd blocks of size} \ge 3\}$ matching edges, and the exact values at
computable sizes again lose only one edge per block beyond the second
(e.g.\ $\max_\pi\mu = 26 = n-3$ for the five-block chain at $n = 29$).
...
Third, the machine computations of Section~\ref{sec:comp} support promoting
...
%% [E6 REVERSED 2026-07-28, Alejandro] Trap-chain appendix reinstated verbatim
%% from v2 (multi-family f-evidence kept for the E8/Q2 splice; see edit ledger).
...
For $\sigma \in S_n$, the \emph{scrambled ladder} $G(n,\sigma)$ is the graph on $2n$
vertices
