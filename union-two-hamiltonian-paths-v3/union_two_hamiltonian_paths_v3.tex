...
\hypersetup{
  pdftitle={How long a properly colored path must the union of two Hamiltonian paths contain?},
...
%% [E10 2026-07-30, merged 2026-08-04] Title decision: question form (option D) —
%% the paper's declared centerpiece is the open lower-bound problem; the old third
%% clause named the E5-cut section. pdftitle + code/README quote updated in the
%% same pass. (Decided on the laptop 07-30, never pushed; merged post-E27-pull.)
\title{How long a properly colored path must\\
the union of two Hamiltonian paths contain?}
...
\begin{document}
\maketitle
%% [cut 2026-08-04, Alejandro] Periapsis lockup removed from the front page.

\begin{abstract}
%% [E13 draft 2026-07-30, merged 2026-08-04] E5-era scaffolding abstract replaced
%% by the two-paragraph draft (chat-drafted, Alejandro-approved 07-30); the f(n)
%% ending re-aligned to the E27 recast (conj:flinear, "high" wording) at merge
%% time. Final hand-polish remains Alejandro's (E13).
Overlay two Hamiltonian paths on a common vertex set of size $n$, one blue and one
red. How long a properly colored path must the union contain? Every such union is
$H_\sigma$ for a permutation $\sigma$ of
...
prove that $3$ is optimal at every gadget size: the construction cannot be improved
by better gadgets. The problem the paper poses is the lower bound: \emph{does
$\pmin(n) \to \infty$?} Two overlaid spanning paths look far too connected for the
answer to be no, yet no bound growing with $n$ is known, and any such bound must
stop at $O(\sqrt n\,)$.

%% [E13 trim 2026-08-04, Alejandro] ¶2 (the three elaborated open problems) CUT —
%% abstract was oversized; replaced by the one-sentence closer below. The
%% "alternating in color" gloss in ¶1 cut in the same pass.
We close with further open problems, each with exact data behind it.
\end{abstract}
...
f(10) = 8, \quad f(11) = 9, \quad f(12) = 10 ,
\end{gather*}
%% [E27 2026-08-04] Display extended to n = 12 (WP-M); two-value sentence replaced
%% by the exception report; 13..29 bracket clause approved 2026-08-03.
and, in fact, $\max_\pi \mu(\pi) \in \{n-1,\, n\}$ for \emph{every} $\sigma \in S_n$,
$n \le 9$ -- but not beyond: from $n = 10$ a third value appears. Exactly
$8$, $24$, and $1088$ permutations of $S_{10}$, $S_{11}$, $S_{12}$ (respectively)
have $\max_\pi \mu = n-2$, and none does worse through $n = 12$, so
$f(n) = n-2$ there. The smallest examples are the four-block permutations of
shape $(2,3,3,2)$, e.g.\ $\sigma = (1,0,\ 4,3,2,\ 7,6,5,\ 9,8)$, where two
odd blocks each cost a rung; all exceptional permutations satisfy
$\rho(\sigma) \le n-2$, as the conversion inequality forces. The smallest $n$
admitting a three-rung loss lies between $13$ and $29$, the upper end from the
exact integer-programming values for the chain and gadget families. The top
value is exact and structural:
...
gadget family retains high matching content at every point we can compute
exactly. Exact values (integer programming, optimality certified) are
\[
\max_\pi \mu \;=\; n,\ n-1,\ n-2,\ n-3,\ n-4 \qquad \text{for } \sigma_{a,6},\ a = 2,\dots,6,
\]
...
%% [E27 2026-08-04] Open problem RECAST as the linearity conjecture (Alejandro,
%% 2026-08-03); sigma_{6,6} = 32 point added (a = 6); "much closer to n" gloss cut.

\begin{conjecture}\label{conj:flinear}
$f(n) = \Theta(n)$: there is a constant $c > 0$ with $f(n) \ge cn$ for all $n$.
\end{conjecture}

The strongest lower bound known is $f(n) \ge 0.1\, n^{4/5}$
(Appendix~\ref{sec:conversion}), so even linearity is open. Two more ambitious
targets suggest themselves: $f(n) = n - o(n)$ (sublinear loss), and the sharper
$f(n) \ge n - O(\sqrt n)$, which would make the loss of $f$ match the collapse
scale of $\rho$ itself. The exact values above mark the fork: if the
one-edge-per-copy pattern of $\sigma_{a,6}$ persists for every $a$, then
$f(n) \le \tfrac{5}{6}\,n + 2$ infinitely often, refuting both targets while
leaving Conjecture~\ref{conj:flinear} intact; if it saturates, the ambitious
targets come back into play. Settling the pattern for large $a$ -- a single
structured family -- may be more tractable than the conjecture itself.
...
enumeration (the censuses of Appendix~\ref{sec:comp}, logs
\texttt{exh7.txt}--\texttt{exh12.txt}).
%% [E22 2026-07-29] Census extended to n <= 16 from the WP-I threshold censuses.
%% [E23/E26 pre-staged 2026-08-04] Sourcing paragraph in FINAL form (counting
%% census + artifact shipping, census1316/); |D(13..15)| filled; only the
%% |D(16)| cell remains open (landing edit).
...
$\pmin(n)$ and \emph{all} of its minimizers, but not the full distribution.
The values $|D(n)|$ in that range come from a separate exhaustive
\emph{counting census}: one deficiency decision per canonical representative,
accumulated with exact orbit weights, run independently on CPU and GPU with
full agreement at $n = 13, 14$, and reproducing the exhaustive $n \le 12$
counts exactly. Every minimizer found was certified by the same independent
integer-programming formulation used for Proposition~\ref{prop:k3} together
with an independent reference solver, and the counts were certified two-sided
on random samples (deficiency by both certifiers, non-deficiency by explicit
properly colored Hamiltonian paths); the engines, witness lists, and
certificates ship with the ancillary artifact
(\texttt{code/census1316/}). For
...
$13$ & $6{,}227{,}020{,}800$      & $26{,}310{,}368$ & $9$ \\
$14$ & $87{,}178{,}291{,}200$     & $281{,}638{,}656$ & $10$ \\
$15$ & $1{,}307{,}674{,}368{,}000$  & $6{,}454{,}817{,}248$ & $9$ \\
$16$ & $20{,}922{,}789{,}888{,}000$ & $81{,}620{,}723{,}456$ & $10$ \\
\bottomrule
\end{tabular}
\end{center}

Three features of the table are worth recording. First, $\pmin$ is not monotone:
the values at $n = 7, 11, 15$ sit below both neighbors. Second, the deficient
sets remain a small fraction of $S_n$ throughout the counted range -- under
half a percent, though not monotonically ($|D(n)|/n! \approx 0.25\%,\ 0.42\%,\
0.32\%,\ 0.49\%,\ 0.39\%$ at $n = 12, \dots, 16$) -- while their internal
...
%% [E27 2026-08-04] Bridge sentence (WP-M); E26's rewrite must preserve it.
Third, the deficient set is exactly where the ladder parameter of
Appendix~\ref{sec:conversion} dips: $\max_\pi \mu(\pi) \le n-1$ if and only if
$\sigma \in D(n)$ (Observation~\ref{obs:topcontent}), and for $n \le 12$ the dip
is one rung except on the small exceptional sets of Section~\ref{sec:disc}
($8$, $24$, $1088$ permutations at $n = 10, 11, 12$), where it is exactly two.
...
\begin{enumerate}
%% [E26 fold 2026-08-04] Item "Census anchors" folded into the exhaustive-census
%% item below (its only distinct content was Python dual-sourcing of two values).
...
their product.
%% [E26 reword 2026-08-04] "joint (b, rho) distribution" fossil replaced: the
%% item now states what the paper uses (E26 item 2; coordinates with E15).
\item \textbf{Exhaustive small-$n$ censuses} (\texttt{logs/exh7.txt}--%
\texttt{exh12.txt}, C++ solver): exact $\rho(\sigma)$ for every
$\sigma \in S_n$, $7 \le n \le 12$ -- the source of the census table of
...
permutations. The anchors $\pmin(7) = 5$ and $\pmin(8) = 6$ were reproduced
independently by full enumeration with the Python solvers
(\texttt{search\_small.log}).
\item \textbf{The $n = 13$--$16$ censuses} (\texttt{census1316/}): the
threshold censuses determining $\pmin(13..16)$ and all of their minimizers
(CPU and GPU engines, full dual-enumerator agreement at $n = 13, 14$), and
the orbit-weighted counting censuses determining $|D(n)|$ there (dual counts
at $n = 13, 14$; exact reproduction of the exhaustive $n \le 12$ values).
Every minimizer is certified two ways
(\texttt{cert\_rho\_n13..16.log}: reference solver + integer programming,
optimality required) and every count two-sided on random samples
(\texttt{cert\_D\_n13..16.log}). That each census visited every permutation
exactly once is checked without re-enumerating: the number of canonical
representatives it reports must equal the number of orbits of $S_n$ under the
eight symmetries, which Burnside's lemma gives in closed form, and the two
agree exactly for every $n \le 16$ (\texttt{coverage\_certificate.log}).
\texttt{census1316/RESULTS.md} maps the files.
...
%% [E27 2026-08-04] f-item extended with the WP-M decision-over-D(n) method.
processed); and, for the $f(10)$--$f(12)$ values, a pruned decision procedure
for ``$\max_\pi \mu \ge n-1$?'' run over the deficient sets $D(10)$--$D(12)$
only (Observation~\ref{obs:topcontent} covers the rest of $S_n$), validated by
exact agreement with the dynamic program on all of $D(10)$ and $D(11)$, with
every $\max_\pi \mu = n-2$ exception certified independently by the
integer-programming formulation (optimality in all $1120$ cases); an
integer-programming formulation for $\max_\pi \mu$ used for
the structured instances (\texttt{fmu\_cpsat.py}, optimality certified in all
reported cases).

\section*{Acknowledgments}

%% [E16 cut 2026-07-30, merged 2026-08-04] "merges and supersedes the author's two
%% earlier notes ... obtained in a follow-up research campaign" sentence removed
%% (process narration, same genre as the E20 trust/preprint stance cuts).
The proofs and computations were
