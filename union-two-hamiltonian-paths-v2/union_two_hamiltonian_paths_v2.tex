...
Exhaustive computation through $n\le12$ (the range documented in the ancillary
artifact) found $\pmin(n)=\lceil(n+3)/2\rceil$ tight at $n=7,8,11,12$ and slack by $1$
elsewhere, with $\sigma_m$ realizing the bound at $n=4m+3$. Together with
Proposition~\ref{prop:sigmam-lower} this suggested:
...
\subsection{An \texorpdfstring{$n=18$}{n=18} counterexample}\label{subsec:n18}
...
\begin{gather*}
  B_1=\{0,1\},\quad B_2=\{2,3,4\},\quad B_3=\{5\},\quad B_4=\{6,7,8\},\\
  B_5=\{9,10,11\},\quad B_6=\{12\},\quad B_7=\{13,14,15\},\quad B_8=\{16,17\}.
\end{gather*}

\begin{proof}
\emph{Lower bound.} The sequence
\[
  \Pi^\ast=(13,14,15,12,11,6,5,2,1,0)
\]
is a PC path on $10$ vertices in $H_{\sigma^\ast}$. Reading the edges from the
definition of $\sigma^\ast$:
\[
\begin{array}{llll}
\{13,14\}\ \text{doubled} & \{14,15\}\ \text{doubled} & \{12,15\}\ \text{V-only} &
\{11,12\}\ \text{P-only} \\
\{6,11\}\ \text{V-only} & \{5,6\}\ \text{P-only} & \{2,5\}\ \text{V-only} &
\{1,2\}\ \text{P-only} \\
\{0,1\}\ \text{doubled}
\end{array}
\]
(the V-only edges join positions whose $\sigma^\ast$-values are consecutive but whose
positions are not, e.g.\ $\sigma^{\ast-1}(8)=6$, $\sigma^{\ast-1}(9)=11$ for
$\{6,11\}$). Assigning the colors
$\mathrm V,\mathrm P,\mathrm V,\mathrm P,\mathrm V,\mathrm P,\mathrm V,\mathrm P,
\mathrm V$ to the nine edges (the doubled edges take the colors the alternation
requires) gives a valid PC traversal, so $\rho(\sigma^\ast)\ge10$.

\emph{Upper bound.} An exact longest-PC-path search
...
$\sigma^\ast$ is a single-position deletion from $\sigma_4\in S_{19}$: delete the center
...
consecutive non-exposed vertices (in path order) has even length. In particular:
\begin{enumerate}
...
exposed vertices must pair within the run, forcing even length. For (a), the runs have
lengths $j$ and $m-1-j$, both even iff $j$ is even and $m$ is odd; for (b), the runs
have lengths $i$, $j-i-1$, $m-1-j$, all even iff $i$ is even, $j$ is odd, $m$ is even.
\end{proof}
...
\section{Block counts do not bound the longest properly colored path}
\label{sec:blockfail}

Recall from Definition~\ref{def:pmblock} the $\pm1$-block count $b(\sigma)$ and the
two elementary facts that frame the lower-bound question: $\rho \ge \smax$ always, and
$\smax \cdot b(\sigma) \ge n$. Any inequality of the form
$\rho(\sigma) \ge c\, b(\sigma)$ with an absolute $c > 0$ would therefore give
$\pmin(n) \ge c\sqrt n$ and settle Conjecture~\ref{conj:sqrt}. The inequality with
$c = 1$ holds exhaustively for $n \le 12$ (Section~\ref{sec:comp}). This section shows
that the route is closed completely: first we locate the exact threshold where
$\rho \ge b(\sigma)$ fails -- it is $n = 14$, far below the $n = 240$ counterexamples
visible inside the family $\sigma_{a,b}$ -- and then we prove that
\emph{no} positive constant works:
$\inf_\sigma \rho(\sigma)/b(\sigma) = 0$. The witnesses are \emph{block-free}
permutations -- $b(\sigma) = n$, every block a singleton -- with
$\rho = \Theta(\sqrt n)$, so the failure is as extreme as it could be.

\subsection{The exact threshold for \texorpdfstring{$\rho \ge b(\sigma)$}{rho >= b}}

\begin{theorem}\label{thm:n14}
Let $\sigma^\dagger = (2,0,3,1,7,5,9,4,8,6,12,10,13,11) \in S_{14}$. Then
$\sigma^\dagger$ is block-free, $b(\sigma^\dagger) = 14$, and
$\rho(\sigma^\dagger) = 13$, so $\rho \ge b(\sigma)$ fails at $n = 14$. Moreover,
exhaustive computation (Section~\ref{sec:comp}) shows:
\begin{enumerate}
\item[(a)] $\rho(\sigma) \ge b(\sigma)$ for every $\sigma \in S_n$ with $n \le 12$;
\item[(b)] every one of the $831{,}283{,}558$ block-free permutations in $S_{13}$ is
properly-colored Hamiltonian ($\rho = 13$);
\item[(c)] at $n = 14$, exactly $16$ block-free permutations are not properly-colored
Hamiltonian; each of them violates $\rho \ge b(\sigma)$.
\end{enumerate}
Hence $n = 14$ is the exact threshold within the block-free class, and no
counterexample of any kind exists for $n \le 12$. (Whether a counterexample with
$b(\sigma) < n$ exists at $n = 13$ we have not determined.)
\end{theorem}

\begin{proof}
Block-freeness of $\sigma^\dagger$ is a direct check: the consecutive value
differences are
\[
-2,\ 3,\ -2,\ 6,\ -2,\ 4,\ -5,\ 4,\ -2,\ 6,\ -2,\ 3,\ -2,
\]
none equal to $\pm1$, so every $\pm1$-block is a singleton and
$b(\sigma^\dagger) = 14$. The values
$\rho(\sigma^\dagger) = 13$ and the census counts in (a)--(c) are exhaustive
computations, reproducible from the ancillary artifact; the single-instance value
$\rho(\sigma^\dagger) = 13$ is confirmed by the reference solvers of
Section~\ref{sec:comp} (a PC path on $13$ vertices exists and the exact search over
all states returns no larger one).
\end{proof}

\begin{remark}
Theorem~\ref{thm:n14}(b) and (c) also settle the extreme case
$b(\sigma) = n$ left open in earlier discussions of this problem: block-free
permutations need \emph{not} be properly-colored Hamiltonian, but the first failures
occur only at $n = 14$. The block-reversal data of Section~\ref{sec:halfbound}
(the half-bound, true through $n = 12$, false at $n = 18$) and
Theorem~\ref{thm:n14} (the block bound, true through $n \le 12$, false at $n = 14$)
are two instances of the same lesson: in this problem family, exhaustive verification
at small $n$ is weak evidence.
\end{remark}

\subsection{Block-free chains with \texorpdfstring{$\rho = \Theta(\sqrt n)$}{rho =
Theta(sqrt n)}}

The threshold witnesses have deficit $1$. To make the failure asymptotic we need
block-free permutations whose $\rho$ grows much more slowly than $n$; we build them as
inflations whose blocks are copies of a \emph{modular staircase} pattern.

\begin{definition}[Staircase pattern]\label{def:stair}
Call $s$ \emph{admissible} if $s \ge 9$ is odd and $s \not\equiv 1 \pmod 3$
(equivalently $s \equiv 3, 5 \pmod 6$). For admissible $s$ put $m = s - 1$ (so $m$ is
even and $3 \nmid m$) and define $\theta_s \in S_s$ by
\[
\theta_s(0) = m, \qquad \theta_s(i) = 3i \bmod m \ \ (1 \le i \le m-1), \qquad
\theta_s(m) = 0 .
\]
For $k \ge 1$ let $\Theta_{s,k} := \mathrm{id}_k[\theta_s, \dots, \theta_s] \in S_{sk}$
be the $k$-fold chain (Definition~\ref{def:inflation} with identity outer
permutation). We write \emph{piece} $j$ ($1 \le j \le k$) for the $j$-th position
block, \emph{vertex $i$ of piece $j$} for position $(j-1)s + i$, and \emph{junction}
$j$ for the boundary between pieces $j$ and $j+1$.
\end{definition}

For example $\theta_9 = (8,3,6,1,4,7,2,5,0)$ and
$\theta_{11} = (10,3,6,9,2,5,8,1,4,7,0)$.

\begin{lemma}[Staircase basics]\label{lem:stair}
Let $s$ be admissible and $k \ge 1$. Then:
\begin{enumerate}
\item[(a)] $\theta_s$ is a permutation of $\{0, \dots, m\}$;
\item[(b)] $\theta_s$ is block-free, and so is every chain:
$b(\Theta_{s,k}) = sk$;
\item[(c)] the cross edges of $\Theta_{s,k}$ at junction $j$ ($1 \le j \le k-1$) are
exactly: the P-edge joining vertex $m$ of piece $j$ to vertex $0$ of piece $j+1$, and
the V-edge joining vertex $0$ of piece $j$ to vertex $m$ of piece $j+1$. Consequently
every cross edge incident to piece $j$ attaches at vertex $0$ or vertex $m$ of the
piece: at vertex $0$, the P-edge of junction $j-1$ and the V-edge of junction $j$; at
vertex $m$, the V-edge of junction $j-1$ and the P-edge of junction $j$ (omitting
nonexistent junctions).
\end{enumerate}
\end{lemma}

\begin{proof}
(a) Multiplication by $3$ is a bijection on $\mathbb Z_m$ since $3 \nmid m$, and it
maps $\{1, \dots, m-1\}$ onto itself (only $0 \mapsto 0$); together with
$\theta_s(0) = m$ and $\theta_s(m) = 0$ all values $0, \dots, m$ occur exactly once.

(b) Consecutive value differences of $\theta_s$: for $1 \le i \le m-2$,
$\theta_s(i+1) - \theta_s(i) \equiv 3 \pmod m$, so the difference is $3$ or $3 - m$;
at the ends, $\theta_s(1) - \theta_s(0) = 3 - m$ and
$\theta_s(m) - \theta_s(m-1) = -(m-3)$ (since $3(m-1) \bmod m = m - 3$). As
$m \ge 8$, no difference is $\pm1$, so every $\pm1$-block of $\theta_s$ is a
singleton. In the chain, differences within a piece equal differences of $\theta_s$
(the values shift by a constant per piece), and across junction $j$ the difference is
$\bigl(js + m\bigr) - (j-1)s = s + m \ge 2$ -- the last position of piece $j$ holds
the piece's bottom value $(j-1)s$ and the first position of piece $j+1$ holds that
piece's top value $js + m$. So every block of $\Theta_{s,k}$ is a singleton.

(c) By Lemma~\ref{lem:crosscount} the cross edges are one P-edge and one V-edge per
junction. The P-edge joins the last position of piece $j$ (vertex $m$) to the first
position of piece $j+1$ (vertex $0$); its value gap is $s + m \ge 2$, so it is P-only.
The V-edge joins $\inv\Theta(js - 1)$ to $\inv\Theta(js)$, i.e.\ the position of piece
$j$'s top value to the position of piece $j+1$'s bottom value; within a piece the top
value sits at vertex $0$ ($\theta_s(0) = m$) and the bottom value at vertex $m$
($\theta_s(m) = 0$), so the V-edge joins vertex $0$ of piece $j$ to vertex $m$ of
piece $j+1$, a non-adjacent position pair: V-only. The attachment statement reads off
these two descriptions at junctions $j-1$ and $j$.
\end{proof}

\begin{lemma}[Parity]\label{lem:stairparity}
$H_{\theta_s}$ is bipartite with parts the even and the odd positions: every edge
joins an even position to an odd one. Consequently every PC path in $H_{\theta_s}$
between vertices $0$ and $m$ has an even, positive number of edges, and hence its
first and last edges have \emph{different} colors.
\end{lemma}

\begin{proof}
P-edges join consecutive positions. For the V-edges, let $g := 3^{-1} \bmod m$; then
$g$ is odd, since $m$ is even and an even $g$ would make $3g \bmod m$ even, while
$3g \equiv 1$. The V-edge of value $v$ joins $\inv{\theta_s}(v)$ to
$\inv{\theta_s}(v+1)$. For $1 \le v \le m-2$ these are $gv \bmod m$ and
$g(v+1) \bmod m$, whose difference is $\equiv g \pmod m$; since $m$ is even, adding an
odd residue modulo $m$ flips the parity of a position. For $v = 0$ the edge is
$\{m,\, g\}$ and for $v = m-1$ it is $\{m - g,\, 0\}$; in both, one entry is even
($m$, $0$) and the other odd ($g$, $m-g$). So every edge flips parity. Vertices $0$
and $m$ are both even, so any path between them has an even number of edges, and it is
positive since $0 \ne m$. A properly colored path alternates colors, so with an even
edge count its first and last edges differ in color.
\end{proof}

\begin{theorem}[Chain collapse]\label{thm:stairchain}
For every admissible $s$ and every $k \ge 4$,
\[
  \rho(\Theta_{s,k}) \;\le\; 4s + 2(k - 4),
  \qquad\text{while}\qquad b(\Theta_{s,k}) = sk .
\]
\end{theorem}

\begin{proof}
Let $Q$ be a PC path in $H_{\Theta_{s,k}}$, decomposed into visits as in
Lemma~\ref{lem:visits}. Each visit lies in one piece; classify the visits:
an \emph{end visit} contains an endpoint of $Q$ (it has at most one of entry/exit
cross edge); every other visit has both an entry and an exit cross edge, which attach
to its piece and hence belong to the piece's two junctions
(Lemma~\ref{lem:stair}(c)) -- call the visit a \emph{return} if they belong to the
same junction and a \emph{through visit} otherwise.

\emph{Through visits have size $1$.} Let $W$ be a through visit of piece $j$, with
entry and exit cross edges at junctions $j-1$ and $j$ in some order. By
Lemma~\ref{lem:ports}(a) the piece induces a color-isomorphic copy of $H_{\theta_s}$,
and by Lemma~\ref{lem:stair}(c) the four cross edges attach at the piece's vertices
$0$ and $m$ with the colors listed there. Four cases.
If the entry is the junction-$(j-1)$ P-edge (at vertex $0$) and the exit is the
junction-$j$ P-edge (at vertex $m$), then $W$ is a PC path from $0$ to $m$; if it has
$\ge 2$ vertices, the PC condition at its ends forces its first and its last internal
edge to be V-colored -- equal colors -- contradicting Lemma~\ref{lem:stairparity};
and it cannot have one vertex since $0 \ne m$. The V-in/V-out case is symmetric (both
end edges P-colored). If the entry is the junction-$(j-1)$ P-edge (at vertex $0$) and
the exit is the junction-$j$ V-edge (also at vertex $0$), then the first and last
vertex of $W$ coincide, so $W$ is the single vertex $0$; the PC condition there holds
as the two cross edges have different colors. Symmetrically, V-in at $m$ with P-out at
$m$ gives the single vertex $m$. So every through visit that exists at all has exactly
one vertex.

\emph{Returns occur only at the extremities.} Suppose a visit $R$ of piece $j$ enters
and exits through the two cross edges of junction $j-1$ (a \emph{left return}). Those
are the only two edges crossing junction $j-1$, and a path uses each edge at most
once, so $Q$ decomposes as $A \cdot e_{\mathrm{in}} \cdot R \cdot e_{\mathrm{out}}
\cdot B$ where $A$ and $B$ never cross junction $j-1$ and end (resp.\ begin) in piece
$j-1$. Hence $A$ and $B$ lie entirely in pieces $< j$, and all vertices of $Q$ in
pieces $\ge j$ belong to $R$, which lies in piece $j$: piece $j$ is the last piece $Q$
touches. Two left returns would both have to sit at that last piece and jointly use
four edges of one junction, which has only two; so $Q$ has at most one left return,
and symmetrically at most one \emph{right return}, whose piece is the first piece $Q$
touches.

\emph{Charging.} The pieces touched by $Q$ form a contiguous range (crossing between
pieces uses a cross edge of the junction between them). Call a touched piece
\emph{special} if it contains an endpoint of $Q$ or hosts a return: there are at most
$2 + 1 + 1 = 4$ special pieces. A special piece contributes at most $s$ vertices (its
size). Every visit to a non-special touched piece is a through visit of size $1$, and
such a piece has at most $4$ incident cross edges, each usable once, with each through
visit consuming two: at most $2$ visits, hence at most $2$ vertices. With at most $k$
touched pieces in all,
\[
\rho(Q) \;\le\; 4s + 2(k - 4) \qquad (k \ge 4),
\]
where the case of fewer than four special or touched pieces only improves the count.
The block count is Lemma~\ref{lem:stair}(b).
\end{proof}

\begin{remark}[Exactness; data]\label{rem:stairdata}
Exact computation (Section~\ref{sec:comp}) gives $\rho(\Theta_{9,k}) = 9k$ for
$k \le 4$ (the chains are properly-colored Hamiltonian up to four pieces: the
transient is real) and, by certified-optimal constraint-solver runs,
$\rho(\Theta_{9,k}) = 38, 40, 42$ at $k = 5, 6, 7$ ($n = 45, 54, 63$) and
$\rho(\Theta_{15,4}) = 60$, $\rho(\Theta_{15,5}) = 62$: every certified value equals
$4s + 2(k-4)$, so the bound of Theorem~\ref{thm:stairchain} is attained at every point
checked. We use only the inequality.
\end{remark}

\begin{theorem}[No block floor]\label{thm:noblockfloor}
$\displaystyle \inf_{\sigma} \frac{\rho(\sigma)}{b(\sigma)} = 0$. In particular, for
every constant $c > 0$ the inequality $\rho(\sigma) \ge c\, b(\sigma)$ fails, and for
every constant $C$ the inequality $\rho(\sigma) \ge b(\sigma) - C$ fails. Moreover
there are block-free permutations ($b(\sigma) = n$) with
$\rho(\sigma) = O(\sqrt n\,)$.
\end{theorem}

\begin{proof}
Take $k = s$ in Theorem~\ref{thm:stairchain}: the chain $\Theta_{s,s}$ on $n = s^2$
vertices is block-free with
$\rho/b \le (6s - 8)/s^2 \to 0$ as admissible $s \to \infty$, proving the infimum
statement and the failure of $\rho \ge c\,b$ for every $c$. The deficit
$b - \rho \ge s^2 - 6s + 8 \to \infty$ kills $\rho \ge b - C$ for every $C$. For the
last statement, $\rho(\Theta_{s,s}) \le 6s - 8 = 6\sqrt n - 8$ with
$b(\Theta_{s,s}) = n$.
\end{proof}

\begin{corollary}[Upper bound along the chains]\label{cor:stairconst}
For every admissible $s$, taking $k = 2s$ and $N = sk = 2s^2$,
\[
  \pmin(N) \;\le\; \rho(\Theta_{s,2s}) \;\le\; 4s + 2(2s - 4) \;=\; 4\sqrt2\,\sqrt N - 8 ,
\]
so $\pmin(N) \le 4\sqrt2\,\sqrt N - 8$ on the infinite set
$\{2s^2 : s \text{ admissible}\}$.
\end{corollary}

\begin{remark}[Two extremes with one growth rate]\label{rem:extremes}
The leading constant $4\sqrt 2 \approx 5.66$ coincides with what the optimized gadget
family already gives along its own special values of $n$
(Remark~\ref{rem:constants}). What is new is where the family sits:
$\sigma_{a,b}$ has $b(\sigma) \approx 4\sqrt n$ blocks and
$\rho/b \to \tfrac12$, while the staircase chains sit at the opposite extreme
$b(\sigma) = n$ with $\rho/b \to 0$. Permutations at \emph{both} ends of the
block-count spectrum thus collapse $\rho$ to $\Theta(\sqrt n)$ -- which is why no
statistic of the block structure alone can force long properly colored paths, and why
Conjecture~\ref{conj:sqrt}, if true, is best possible: no lower bound of order
$\omega(\sqrt n)$ can hold.
\end{remark}
